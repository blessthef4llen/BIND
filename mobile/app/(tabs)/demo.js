...
function StepCard({ step }) {
  ...
  useEffect(() => {
    Animated.timing(anim, { toValue: step.state === 'idle' ? 0 : 1, duration: 300, useNativeDriver: false }).start();
  }, [step.state]);
  ...
  return (
    <Animated.View style={[styles.stepCard, { backgroundColor: bgColor, borderColor: step.state !== 'idle' ? 'rgba(212,43,43,0.25)' : 'rgba(255,255,255,0.06)' }]}>
      ...
      <View style={{ flex: 1 }}>
        <Text style={[styles.stepName, { color: nameCl }]}>{step.name}</Text>
        <Text style={[styles.stepDesc, { color: descCl }]}>{step.desc}</Text>
      </View>
    </Animated.View>
  );
}
    ...
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
      ...
      <View style={{ flex: 1, flexDirection: 'row', gap: 3 }}>
        {Array.from({ length: 10 }, (_, i) => i + 1).map(n => (
          <Pressable key={n} style={[styles.sevSeg, n <= value && styles.sevSegOn]} onPress={() => onChange(n)} />
        ))}
      </View>
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        <View style={styles.form}>
          ...
          <TextInput style={styles.darkInput} value={area} onChangeText={setArea} placeholder="Body area" placeholderTextColor="rgba(255,255,255,0.25)" />
          <TextInput style={[styles.darkInput, { minHeight: 60 }]} value={symptom} onChangeText={setSymptom} placeholder="Symptom" placeholderTextColor="rgba(255,255,255,0.25)" multiline />
